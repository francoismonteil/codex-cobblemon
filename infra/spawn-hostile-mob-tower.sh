...
# Spawns a vanilla hostile mob tower either near a player or at absolute
# coordinates, using only server console commands.
...
Usage:
  $0 [--player <name> | --at <x> <y> <z>] [--dx <int>] [--dy <int>] [--dz <int>]
     [--floors <int>] [--no-clear] [--forceload] [--json-out <path>]
EOF
}
...
resolve_player_origin() {
  ...
  if [[ -z "${px:-}" || -z "${py:-}" || -z "${pz:-}" ]]; then
    echo "Failed to parse player position: ${pos_line}" >&2
    exit 1
  fi
...
NO_CLEAR_PY="False"
FORCELOAD_PY="False"
if [[ "${no_clear}" == "true" ]]; then NO_CLEAR_PY="True"; fi
...
run_fill() {
  ...
  cmd "fill ${x1} ${y1} ${z1} ${x2} ${y2} ${z2} ${block} replace"
}
...
run_generated_build() {
  local include_clear_py="True"
  if [[ "${no_clear}" == "true" ]]; then
    include_clear_py="False"
  fi
...
from hostile_mob_tower_spec import build_operations

for op in build_operations((${X0}, ${Y0}, ${Z0}), ${floors}, include_clear=${include_clear_py}):
    if op.kind == "fill":
        print(f"fill {op.x1} {op.y1} {op.z1} {op.x2} {op.y2} {op.z2} {op.block} replace")
    else:
        print(f"setblock {op.x1} {op.y1} {op.z1} {op.block} replace")
PY
...
tell_target "Hostile tower: generation start (floors=${floors})."
...
write_json
tell_target "Hostile tower: done."
echo "OK"
