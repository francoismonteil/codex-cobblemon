...
# Checks the cumulative current live baseline plus addon rollout pack.
#
# Usage:
#   ./infra/mods-check-addons-rollout.sh --through-lot 1
#   ./infra/mods-check-addons-rollout.sh --through-lot 8 --mods-dir ./data/mods

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "${SCRIPT_DIR}/.." && pwd)"
cd "${REPO_ROOT}"

PY_SCRIPT="${REPO_ROOT}/tools/check_addons_rollout.py"
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    *)
      ...
      echo "Usage: ./infra/mods-check-addons-rollout.sh --through-lot <1..8> [--mods-dir <path>]" >&2
      exit 1
      ;;
  esac
done
...
if [[ -z "${through_lot}" ]]; then
  echo "Missing required argument: --through-lot <1..8>" >&2
  exit 1
fi
...
case "${through_lot}" in
  1|2|3|4|5|6|7|8) ;;
  *)
    echo "Invalid lot number: ${through_lot} (expected 1..8)" >&2
    exit 1
    ;;
esac
...
args=(
  "--write"
  "--out" "${REPO_ROOT}/audit/addons-server-mods-check-lot${through_lot}.json"
)
